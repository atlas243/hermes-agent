...
class TestUpdateMergesMainIntoCustomizations:
    """After updating main, the customizations branch must merge origin/main
    into itself.  Without this step the customizations branch silently falls
    behind upstream and never picks up fixes (regression: codex backfill
    #5689 took 11+ days to reach the live checkout)."""

    def _branch_safe_side_effect(self, merge_rc: int = 0):
        """Subprocess mock that simulates the customizations-branch update flow."""
        calls: list = []
        ...
        def side_effect(cmd, **kwargs):
            ...
            # git rev-parse --abbrev-ref HEAD → on customizations
            if "rev-parse" in joined and "--abbrev-ref" in joined:
                return subprocess.CompletedProcess(cmd, 0, stdout="blaize-customizations\n", stderr="")
            # git rev-parse --verify origin/main → exists
            if "rev-parse" in joined and "--verify" in joined:
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
            # git rev-list HEAD..origin/main --count → 3 new commits
            if "rev-list" in joined and "--count" in joined:
                return subprocess.CompletedProcess(cmd, 0, stdout="3\n", stderr="")
            # git merge --no-edit origin/main → use the parameterized rc
            if "merge" in joined and "origin/main" in joined and "--abort" not in joined:
                stderr = "" if merge_rc == 0 else "CONFLICT (content): Merge conflict\n"
                return subprocess.CompletedProcess(cmd, merge_rc, stdout="", stderr=stderr)
            # systemctl list-units, is-active → no service running
            if "systemctl" in joined and ("list-units" in joined or "is-active" in joined):
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
            # default: success
            return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    ...
    @patch("shutil.which", return_value=None)
    @patch("subprocess.run")
    def test_clean_merge_runs_after_branch_restore(
        self, mock_run, _mock_which, mock_args, capsys, monkeypatch,
    ):
        """When on customizations branch and main has new commits, update should
        merge origin/main into customizations after restoring the branch."""
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)

        side_effect, calls = self._branch_safe_side_effect(merge_rc=0)
        ...
        captured = capsys.readouterr().out
        assert "Merging origin/main into 'blaize-customizations'" in captured
        assert "Merged origin/main into 'blaize-customizations'" in captured

        merge_calls = [c for c in calls if len(c) >= 3 and c[1] == "merge" and "origin/main" in c]
        assert len(merge_calls) >= 1, f"Expected at least one git merge call, got: {merge_calls}"

    @patch("shutil.which", return_value=None)
    @patch("subprocess.run")
    def test_conflict_aborts_merge_and_blocks_auto_restart(
        self, mock_run, _mock_which, mock_args, capsys, tmp_path, monkeypatch,
    ):
        """When the auto-merge has conflicts, the update must abort the merge,
        leave a clean working tree, surface to the user, and block auto-restart
        even on the launchd path."""
        plist_path = tmp_path / "ai.hermes.gateway.plist"
        plist_path.write_text("<plist/>")
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: plist_path)

        side_effect, calls = self._branch_safe_side_effect(merge_rc=1)
        mock_run.side_effect = side_effect
        ...
        captured = capsys.readouterr().out
        assert "Could not auto-merge origin/main" in captured
        assert "git merge origin/main" in captured

        abort_calls = [c for c in calls if len(c) >= 3 and c[1] == "merge" and "--abort" in c]
        assert len(abort_calls) == 1, f"Expected exactly one git merge --abort, got: {abort_calls}"
