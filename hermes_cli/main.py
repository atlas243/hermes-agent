    ...
    try:
        ...
        if commit_count == 0:
            ...
            if should_restore_original_branch:
                ...
                # Even when main has no new upstream commits, the
                # customizations branch may still be behind main itself.
                # Merge so it actually catches up.
                if current_branch == EXPECTED_LIVE_BRANCH:
                    behind = subprocess.run(
                        git_cmd + ["rev-list", "--count", "HEAD..origin/main"],
                        cwd=PROJECT_ROOT, capture_output=True, text=True, check=False,
                    )
                    if behind.returncode == 0 and behind.stdout.strip().isdigit() and int(behind.stdout.strip()) > 0:
                        print(f"→ '{EXPECTED_LIVE_BRANCH}' is {behind.stdout.strip()} commit(s) behind origin/main; merging...")
                        merge_result = subprocess.run(
                            git_cmd + ["merge", "--no-edit", "origin/main"],
                            cwd=PROJECT_ROOT, capture_output=True, text=True,
                        )
                        if merge_result.returncode == 0:
                            print(f"  ✓ Merged origin/main into '{EXPECTED_LIVE_BRANCH}'")
                        else:
                            subprocess.run(
                                git_cmd + ["merge", "--abort"],
                                cwd=PROJECT_ROOT, capture_output=True, text=True, check=False,
                            )
                            print(f"  ⚠ Could not auto-merge origin/main into '{EXPECTED_LIVE_BRANCH}': conflicts.")
                            print(f"    Resolve manually: cd {PROJECT_ROOT} && git merge origin/main")
        ...
        if should_restore_original_branch:
            ...
            # Merge origin/main into the customizations branch so it
            # actually picks up upstream changes.  Without this, the
            # customizations branch silently falls behind main over time
            # and never receives upstream fixes (regression history:
            # codex backfill #5689 took 11+ days to reach this checkout).
            if current_branch == EXPECTED_LIVE_BRANCH:
                print(f"→ Merging origin/main into '{EXPECTED_LIVE_BRANCH}'...")
                merge_result = subprocess.run(
                    git_cmd + ["merge", "--no-edit", "origin/main"],
                    cwd=PROJECT_ROOT, capture_output=True, text=True,
                )
                if merge_result.returncode == 0:
                    ...
                    # Merge conflict — abort to leave a clean working tree,
                    # surface to the user, and block the gateway auto-restart
                    ...
                    print(f"  ⚠ Could not auto-merge origin/main into '{EXPECTED_LIVE_BRANCH}': conflicts.")
                    print(f"    Resolve manually: cd {PROJECT_ROOT} && git merge origin/main")
                    print(f"    Skipping gateway auto-restart until '{EXPECTED_LIVE_BRANCH}' is reconciled.")
                    should_auto_restart_gateway = False
