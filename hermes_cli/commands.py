...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("resume", "Resume a previous session by title or ID", "Session",
               args_hint="[name]"),
...
def telegram_menu_commands(
    ...
    # Reorder skills: priority_skills first (preserving user order), then rest.
    if priority_skills:
        # Telegram command names use underscores (no hyphens allowed).
        _prio_tg = [s.replace("-", "_") for s in priority_skills]
        _prio_set = set(_prio_tg)
        prio_entries = [e for e in entries if e[0] in _prio_set]
        rest_entries = [e for e in entries if e[0] not in _prio_set]
        _prio_order = {name: i for i, name in enumerate(_prio_tg)}
        prio_entries.sort(key=lambda e: _prio_order.get(e[0], 999))
        entries = prio_entries + rest_entries
