...
class AIAgent:
    ...
    def touch_activity(self) -> None:
        """Public wrapper for external callers (delegate_tool, gateway/run.py).

        Delegates to ``_touch_activity`` so ``_last_activity_desc`` stays in
        sync.  The gateway's progress-aware timeout monitor polls
        ``_last_activity_ts`` to distinguish idle agents from actively
        working ones.
        """
        self._touch_activity("external activity")
    ...
    def run_conversation(
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            try:
                ...
                self._touch_activity("api response received")
                try:
                    from hermes_cli.plugins import invoke_hook as _invoke_hook
